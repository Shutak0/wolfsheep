...
function getReplayDelay(moves,mi,isExport){var defDelay=isExport?200:500;var wallDelay=isExport?400:467;if(mi<1){return Math.round(333/(mi<4?2:1));}var m=moves[mi];if(m.type==='wall'){return Math.round(wallDelay/(mi<4?2:1));}var streak=0;for(var k=mi;k>=0;k--){var mk=moves[k];if(mk.type==='move')streak++;else break;}var isMoveSeries=(streak>=2);var baseDelay=isMoveSeries?100:defDelay;var mult=isMoveSeries?Math.min(streak/2,4):(mi<4?2:1);return Math.round(baseDelay/mult);}
