...
(function () {
    ...
    function getReplayDelay(moves,mi,isExport){var baseDelay;var isMoveSeries=false;var defDelay=isExport?200:500;var wallDelay=isExport?400:467;if(mi<1){baseDelay=333;}else{var p=moves[mi].player,opp=1-p;var ourMoves=0,ourWalls=0;for(var k=mi;k>=0;k--){var mk=moves[k];if(mk.player!==p)break;if(mk.type==='move')ourMoves++;else if(mk.type==='wall')ourWalls++;}var ourMixed=(ourMoves>0&&ourWalls>0);var oppMoves=0,oppWalls=0;for(var j=mi-1;j>=0;j--){var mj=moves[j];if(mj.player!==opp)continue;for(var q=j;q>=0;q--){var mq=moves[q];if(mq.player!==opp)break;if(mq.type==='move')oppMoves++;else if(mq.type==='wall')oppWalls++;}break;}var oppMixed=(oppMoves>0&&oppWalls>0);if(!ourMixed&&!oppMixed&&ourMoves>=6&&oppMoves>=6){baseDelay=100;isMoveSeries=true;}else if(!ourMixed&&!oppMixed&&ourWalls>=2&&oppWalls>=2)baseDelay=wallDelay;else if(!ourMixed&&!oppMixed&&ourMoves>=2&&oppMoves>=2){baseDelay=200;isMoveSeries=true;}else baseDelay=defDelay;}var mult;if(isMoveSeries)mult=5.0;else if(mi<4)mult=2.0;else mult=1.0;return Math.round(baseDelay/mult);}
    ...
    function doExportVideo(speedMultiplier,customPhrase){
        ...
        function fallbackWebMExport(){
            ...
            var delay=getReplayDelay(mo,si-1,true)/speedMultiplier;replayTimer=setTimeout(step,Math.max(delay,50));}
